...
const planRegionalNetwork = () => {
    ...
    if (
        ...
        networkInputs.sizeSubnet.result = networkInputs.sizeSubnet.count;
    ...
    // regionalSize == vnetNumber * subnetPerVnetNumber? * subnetSize
    if (
        (networkInputs.numberSubnetsPerVirtualNetwork.value <= 0) &&
        (networkInputs.sizeVirtualNetwork.value <= 0 || networkInputs.sizeVirtualNetwork.count <= 0)
    ) {

        console.log("second")

        const numberSubnetsPerVirtualNetwork = sizeRegional / ( 
            networkInputs.numberVirtualNetworks.value * networkInputs.sizeSubnet.count
        )

        const sizeVirtualNetwork = numberSubnetsPerVirtualNetwork * networkInputs.sizeSubnet.count

        console.log("Number of Subnets Per Virtual Network", numberSubnetsPerVirtualNetwork)
        console.log("Size of Virtual Networks", sizeVirtualNetwork)
        console.log('Mask(s) for Virtual Networks', countToMasks(sizeVirtualNetwork))

        networkInputs.numberVirtualNetworks.result = networkInputs.numberVirtualNetworks.value;
        networkInputs.sizeVirtualNetwork.result = sizeVirtualNetwork;
        networkInputs.numberSubnetsPerVirtualNetwork.result = numberSubnetsPerVirtualNetwork;
        networkInputs.sizeSubnet.result = networkInputs.sizeSubnet.count;
    ...
    // regionalSize == vnetNumber * subnetPerVnetNumber * subnetSize?
    if (
        networkInputs.sizeSubnet.value <= 0 || networkInputs.sizeSubnet.count <= 0 &&
        (networkInputs.sizeVirtualNetwork.value <= 0 || networkInputs.sizeVirtualNetwork.count <= 0)
    ) {

        console.log("third")

        const sizeSubnet = sizeRegional / ( 
            networkInputs.numberVirtualNetworks.value * networkInputs.numberSubnetsPerVirtualNetwork.value
        )

        const sizeVirtualNetwork = networkInputs.numberSubnetsPerVirtualNetwork.value * sizeSubnet

        console.log("Size of Subnets", sizeSubnet)
        console.log('Mask(s) for Subnet', countToMasks(sizeSubnet))
        console.log("Size of Virtual Networks", sizeVirtualNetwork)
        console.log('Mask(s) for Virtual Networks', countToMasks(sizeVirtualNetwork))
        ...
        networkInputs.sizeVirtualNetwork.result = sizeVirtualNetwork;
        networkInputs.numberSubnetsPerVirtualNetwork.result = networkInputs.numberSubnetsPerVirtualNetwork.value;
        networkInputs.sizeSubnet.result = sizeSubnet;
    ...
    <div class="flex flex-col gap-8 pt-5">
            ...
            <div class="flex flex-col gap-5 w-56">
                ...
                <InputGroup>
                    ...
                    <Select class="w-min" v-model="networkInputs.sizeVirtualNetwork.value"
                        ...
                        :invalid="networkInputs.sizeVirtualNetwork.invalid"
                        @focusout="networkInputs.sizeVirtualNetwork.clicked = true;" />
                </InputGroup>
            </div>
        </div>
        <div class="flex gap-8">
            ...
            <div class="flex flex-col gap-5 w-56">
                ...
                <InputGroup>
                    ...
                    <Select class="w-min" v-model="networkInputs.sizeSubnet.value"
                        ...
                        :invalid="networkInputs.sizeSubnet.invalid"
                        @focusout="networkInputs.sizeSubnet.clicked = true;" />
                </InputGroup>
            </div>
        </div>
        ...
        <div v-if="networkInputs.plan" class="flex flex-col gap-3">
            ...
            <div>
                ...
                <div v-if="!ipAddressRangeRegional.inputToggle.value" class="pl-4">Range: {{
                    ipAddressRangeRegional.start.value }} - {{ ipAddressRangeRegional.end.value }}</div>
                <div v-else class="pl-4">CIDR: {{ ipAddressRangeRegional.ipAddress.value }}{{
                    ipAddressRangeRegional.cidr.value }}</div>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="pl-4"> {{ countToMasks(networkInputs.sizeSubnet.result) }} </div>
